...
- **Pas de bornes maximales** sur les champs numériques (poids, glucides/100g) — une faute de frappe (ex. un zéro en trop) pouvait fausser une dose sans avertissement. Ajout d'une borne physique stricte (glucides/100g ≤ 100, impossible autrement) et d'une borne haute sur les poids (`MAX_WEIGHT_G`, actuellement 1000g/1kg, ajustable dans `src/lib/insulin.ts`).
...
- **Écrans "options" ajoutés** dans Réglages : Aide (présentation/mode d'emploi), Contact (email + partage), À propos, Mentions légales (confidentialité + CGU + avertissement médical). Pas d'écran "noter l'app" (l'app n'a pas encore d'ID App Store/Play Store configuré).
- **Base Ciqual (Anses) intégrée comme aide à la saisie** : à la création d'un ingrédient, un mini-dropdown auto-suggère jusqu'à 5 aliments Ciqual pertinents pendant la frappe (classés par position du match, pas juste ordre alphabétique), avec une icône loupe pour chercher plus largement dans les ~3270 aliments via une modale plein écran. Ciqual reste une aide ponctuelle, pas une deuxième liste imposée : on choisit une suggestion, elle préremplit nom/glucides/source (`"Ciqual"`), et ça reste un aliment perso ensuite. Table 2025 importée une fois via `scripts/import-ciqual.mjs` (à relancer si Ciqual publie une mise à jour) → `assets/data/ciqual.json`, bundlé en local, aucun appel réseau à l'exécution.
- **`PickerModal` généralisé** (`src/components/PickerModal.tsx`) avec deux props optionnelles rétrocompatibles : `filterItems` (filtre/tri personnalisé, ex. classement par pertinence) et `initialQuery` (recherche pré-remplie à l'ouverture).
- **Placeholders illisibles en dark mode corrigés** : plusieurs champs (`TextInput`) partout dans l'app n'avaient pas de `placeholderTextColor`, rendant le texte d'exemple noir sur fond sombre. Corrigé sur les écrans Peser, Ingrédient, Récipient.
- **Listes récipient/aliment non triées** dans le sélecteur de l'écran Peser (contrairement au ratio, déjà trié par position) : tri alphabétique ajouté sur les deux.
- **Affichage des poids optimisé** : `formatWeight()` bascule automatiquement en kilogrammes au-delà de 1000g (`1500g → "1.5 kg"`) au lieu d'afficher toujours des grammes bruts.
- **Message d'erreur affiché à tort sur champ vide** : le poids brut et la tare manuelle affichaient "poids invraisemblable" dès l'ouverture de l'écran (champ vide interprété comme invalide). Corrigé pour ne montrer l'erreur que sur une vraie valeur hors bornes.
- **Focus auto au lancement corrigé** : le champ "Poids brut" s'ouvrait avec le clavier numérique à chaque lancement de l'app (pas seulement via le raccourci d'icône "Peser" pour lequel c'était pensé). Le focus auto est maintenant conditionné à l'arrivée via ce raccourci précis.
...
- **Export PDF pour le médecin** : mettre en forme l'historique existant (glycémie, glucides, dose repas + correction) en un carnet lisible pour une consultation, plutôt que l'export JSON brut actuel (pensé pour la sauvegarde, pas la lecture humaine). Priorisé après Ciqual, pas encore fait.
- **Graphiques de tendance glycémique** (évoqué puis explicitement écarté) : peu de valeur avec les données actuelles, qui ne capturent la glycémie qu'au moment d'un repas (pas un vrai carnet glycémique à jeun/hors repas) — d'autres outils dédiés font déjà ça bien. À reconsidérer seulement si on étend le tracking à des mesures hors repas.
- **Notes/étiquettes sur les entrées de l'historique** (ex. "avant/après repas", "activité physique") (évoqué puis explicitement écarté) : bonne idée dans l'absolu, mais jugé hors du périmètre de l'app, centrée sur le calcul de dose plutôt qu'un journal de contexte.
- **Base alimentaire Ciqual plus large/catégorisée** : la base est déjà intégrée comme aide à la saisie (voir Corrections). Une évolution possible serait de catégoriser les résultats (ingrédients vs recettes, groupes d'aliments) dans le sélecteur de l'écran Peser plutôt qu'un simple tri alphabétique.
